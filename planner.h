...
public: 
    ...
    bool operator() (const Object& p1, const Object& p2) const
    { 
...
std::vector<std::pair<int,int>> do_Astar(
    ...
    //std::priority_queue<Object, std::vector<Object>, myComparator> OPEN; // priority queue
    ...
    //OPEN.push(Object(g_val,h_val,f_val,Make_Node(robotposeX,robotposeY)));
    ...
    while (OPEN.size() != 0){
        ...
        //auto MIN_OPEN = OPEN.top();
        // auto min_node = MIN_OPEN.Node; // returns the smallest Node by soring the f value
        // auto min_g_val = MIN_OPEN.g;
        auto min_node = OPEN.front().Node;
        auto min_g_val = OPEN.front().g;

        std::pop_heap(OPEN.begin(),OPEN.end(),myComparator());
        OPEN.pop_back();
        //OPEN.pop(); // remove this node from OPEN
        // if goal reached stop the loop
        if (min_node == goal){
            break;
        }
        std::cout<<"size of the queue"<<OPEN.size();
        std::cout<<"min node "<< min_node.first<<" "<<min_node.second<<"and its g val is"<<min_g_val <<"\n";
            ...
            if(CLOSED_table.IsIncluded(Make_Node(newx,newy))== false){ // if not found in closed loop
                ...
                if (newx >= 1 && newx <= x_size && newy >= 1 && newy <= y_size)
                {
                    if (((int)map[GETMAPINDEX(newx,newy,x_size,y_size)] >= 0) && ((int)map[GETMAPINDEX(newx,newy,x_size,y_size)] < collision_thresh))  //if free
                    {
                        ...
                        //std::cout<<"f values at "<<newx<<" "<<newy<<" is "<<f_val<<std::endl;
                        ...
                        for(int i = 0;i < OPEN.size();i++){
                            if(Make_Node(newx,newy)==OPEN[i].Node){
                                ...
                                if(OPEN[i].g > g_val){ 
                                    //std::cout<<"before update fval is  "<<OPEN[i].f<<"\n";  
                                    OPEN[i].g = g_val;
                                    OPEN[i].f = f_val;
                                }
                                count++;
                        ...
                        //OPEN.push(Object(g_val,h_val,f_val,Make_Node(newx,newy)));
