	   ...
	   int* planlength)
{
	...
	*planlength = 0;
    ...
    std::vector< std::array<double,k> > RRT_plan;
    RRT_plan = RRTplanner(map,x_size,y_size,armstart_anglesV_rad,armgoal_anglesV_rad,numofDOFs,plan,planlength);
    int numofsamples = RRT_plan.size();
    ...
    // }
    // numofsamples = (int)(distance/(PI/20));
    // printf("num of samples %d \n",numofsamples);
    ...
    //     return;
    // }
    std::array<double,k> iter;
    *plan = (double**) malloc(numofsamples*sizeof(double*));
    int firstinvalidconf = 1;
    for (int i = 0; i < numofsamples; i++){
        (*plan)[i] = (double*) malloc(numofDOFs*sizeof(double));
        iter = RRT_plan[numofsamples-1-i];
        for(int j = 0; j < numofDOFs; j++){
            (*plan)[i][j] = iter[j];//armstart_anglesV_rad[j] + ((double)(i)/(numofsamples-1))*(armgoal_anglesV_rad[j] - armstart_anglesV_rad[j]);
            //std::cout<<" plan[i][j] "<< (*plan)[i][j] << " where i "<<i<<"where j "<<j<<std::endl;
        }
